...
// relative Importe, damit es auch ohne "@"-Alias sicher läuft
...
export default function Page() {
  ...
  // Trigger für manuelles Neuladen nach Änderungen (Drawer speichert etwas)
  ...
  const feierabendMin = 9 * 60; // 08:00–17:00 = 9h
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        for (const t of tg) {
          ...
          for (let d = 1; d <= dim; d++) {
            ...
            out[t.id][d] = cell
              ...
              : {
                  status: 'verfuegbar',
                  isFriday: isFriday(year, month, d),
                  totalMin: 0,
                  feierabendMin,
                };
          }
        ...
        // Fallback: leeres Board (falls API fehlschlägt)
        setTechs([]);
        setGrid({});
      } finally {
    ...
    // bei year, month ODER refresh neu laden
  }, [year, month, refresh]);

  // nach Änderungen im Drawer das Board neu laden
  ...
  return (
    <div className="space-y-4">
      ...
      <MonthBoard
        ...
        technicians={techs}
        data={grid}
        onCellClick={({ technikerId, technikerName, day }) =>
          setDrawer({ open: true, day, technikerId, technikerName })
        }
      />
      ...
      <TagDrawer
        open={drawer.open}
        onClose={() => setDrawer(v => ({ ...v, open: false }))}
        year={year}
        month={month}
        day={drawer.day}
        technikerId={drawer.technikerId || ''}
        technikerName={drawer.technikerName || ''}
